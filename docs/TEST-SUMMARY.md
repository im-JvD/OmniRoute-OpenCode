...
آخرین اجرای کامل: **۱۰۲ از ۱۰۲ assertion موفق، خروجی 0** (هر ۱۴
gروه تست T1 تا T14).
...
| T13 | اجرای piped مثل `curl \|\| bash` | بایت‌های اسکریپت روی stdin می‌آیند (نه tty) ولی ترمینال کنترل‌کننده موجود است؛ منو و ۵ پرامپت کلید از `/dev/tty` خوانده می‌شوند و نصب کامل با 6/6 راستی‌آزمایی تمام می‌شود. |
