...
آخرین اجرای کامل: **۱۰۶ از ۱۰۶ assertion موفق، خروجی 0** (هر ۱۴
گروه تست T1 تا T14).

```bash
cd tests
bash run-tests.sh            # اجرای هر ۱۴ تست
...
| T12 | حفظ opencode.json موجود | پیکربندی‌ای که provider دیگر (`anthropic`) و کلیدهای سطح‌بالای نامرتبط دارد، پس از install سالم می‌ماند: provider دیگر دست‌نخورده، provider `omniroute` افزوده، `model`ِ سطح‌بالای موجود حفظ می‌شود. |
| T13 | اجرای piped مثل `curl \|\| bash` | بایت‌های اسکریپت روی stdin می‌آیند (نه tty) ولی ترمینال کنترل‌کننده موجود است؛ منو و ۵ پرامپت کلید از `/dev/tty` خوانده می‌شوند و نصب کامل با 6/6 راستی‌آزمایی تمام می‌شود. ذخیره‌ی خودکار نسخه‌ی محلی manager در `~/.omniroute-manager/OmniRoute.sh`، اشاره‌ی shim و خط re-runِ بنر به آن مسیر واقعی (هرگز `bash "bash"`)، و حذف نسخه‌ی ذخیره‌شده در uninstall راستی‌آزمایی می‌شود. |
| T14 | کاتالوگ ۳۰۰ مدل + عملیات manage | رجرسی‌شن برای باگ E2BIGِ `jq --argjson` (کاتالوگ چند مگابایتی واقعی): ۳۰۰۰ مدل در opencode.json؛ عملیات `--status/--down/--up/--restart/--logs`؛ دیسپچ دستور `omni`؛ حذف shim در uninstall. |
